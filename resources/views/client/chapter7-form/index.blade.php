...
@section('content')
    <div class="content-body">
        <div class="dashboard-sec">
            @can('user_chapter7_form_create')
            <div style="margin-bottom: 10px;" class="row">
                <div class="col-lg-12">
                    <a class="btn btn-success" href="{{ route('client.chapter7-form.create') }}">
                        Add New
                    </a>
                </div>
            </div>
                            ...
                            <tbody>
                                @foreach ($part1 as $index => $item)
                                    <tr>
                                        ...
                                        <td>
                                            @can('user_chapter7_form_show')
                                                <a class="btn btn-xs btn-info"
                                                    href="{{ route('client.chapter7-form.show', $item->id) }}">
                                                    View
                                                </a>
                                            @endcan
                                            @can('user_chapter7_form_edit')
                                                <a class="btn btn-xs btn-warning"
                                                    href="{{ route('client.chapter7-form.edit', $item->id) }}">
                                                    Edit
                                                </a>
                                            @endcan
                                            @can('user_chapter7_form_delete')
        ...
        <script>
            function Chapter7_formDelete{{ $item->id }}(id) {
                swal({
                    ...
                    if (willDelete) {
                        var url = '{{ route('client.deleteChapter7Form', ':id') }}';
