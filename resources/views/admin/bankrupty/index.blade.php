...
<div class="content-body">
    ...
    @can('team_create')
        <div style="margin-bottom: 10px;" class="row">
            <div class="col-lg-12">
                <a class="btn btn-success" href="{{ route('admin.bankrupty.create') }}">
                    Add New
                </a>
            </div>
        </div>
                    ...
                    <tbody>
                        @foreach ($team as $index=> $item)
                            <tr>
                                ...
                                <td>
                                    @can('team_edit')
                                        <a class="btn btn-xs btn-info" href="{{ route('admin.bankrupty.edit', $item->id) }}">
                                            Edit
                                        </a>
                                    @endcan
                                    @can('team_delete')
...
<script>
    function teamDelete{{$item->id}}(id){
        swal({
            ...
            if (willDelete) {
                var url = '{{ route("admin.deleteBankrupty", ":id") }}';
